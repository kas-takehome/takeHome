...
**Authenticated synthetic-data demo.** Use synthetic data; never enter real customer data, card numbers, CVV/CVC, PINs, or credentials into disputes or notes.

## Run locally

Use **Node.js 22.12 or later within Node 22 LTS** (`nvm use` uses `.nvmrc`). Install dependencies and configure the admin password once:

```sh
npm install
read -rsp 'Admin password: ' ADMIN_PASSWORD; printf '\n'
printf '%s' "$ADMIN_PASSWORD" | npm run configure:admin
unset ADMIN_PASSWORD
npm run dev
```

The password prompt above uses Bash and does not echo your input. The configuration command stores only a salted scrypt hash in ignored, permission-restricted `.env.local`; it refuses to overwrite an existing file. No default password is shipped. Sign in as **admin** with the password you provisioned.

Open **http://127.0.0.1:5173**. Vite proxies `/api` to Express at **http://127.0.0.1:3001**. Both bind to loopback by default. SQLite creates and seeds `data/disputes.sqlite` on first startup; edits persist across restarts. No external database is needed locally. `better-sqlite3` uses a native addon: if a prebuilt binary isn't available for your OS/Node version, install your platform's C++ build tools and Python.
...
| `ADMIN_PASSWORD_HASH` | Required server-only scrypt hash, generated by `npm run configure:admin`. |
| `TURSO_DATABASE_URL` | Required on Vercel: hosted database URL using `libsql://` or `https://`. |
| `TURSO_AUTH_TOKEN` | Required on Vercel: token for this hosted database. Never use a `VITE_` prefix for secrets. |

The local server loads `.env.local` when present; existing process environment variables take precedence. The Vercel function uses project environment variables only. For a fresh local synthetic dataset, stop the server and choose a new `DATABASE_PATH` rather than overwriting an existing database.

## Authentication and Vercel deployment

`POST /api/auth/login` verifies the admin password using scrypt (N=32768, r=8, p=1, unique 16-byte salt). It issues a random 256-bit opaque session cookie; the database stores only its SHA-256 digest. Sessions expire after eight hours. HTTPS deployments use a `__Host-` cookie with `Secure`, `HttpOnly`, `SameSite=Strict`, and no Domain attribute. `POST /api/auth/logout` revokes the session in shared storage, including across function instances. Changing the username or password hash invalidates previous sessions. `GET /api/auth/session` restores the frontend session; API 401 responses clear the protected workspace. The health endpoint exposes only service status without requiring sign-in.

Login attempts are limited to 10 per IP and 100 globally per 15-minute window, including unsuccessful and successful attempts. Counters live in the same database and survive function restarts. Expired sessions and counters are cleaned during login attempts. API/mutation limits remain per process; add platform-wide limits before production financial-data use.

Vercel's filesystem cannot durably hold SQLite state across instances. The Vercel entry point therefore **requires remote Turso/libSQL storage**, with no local-file fallback. `server/hosted-database.ts` adapts the driver to the same transaction contract used locally. Reads go to the shared primary; there is no embedded replica or stale local cache. Static files come from `dist`; `/api/*` is routed to `api/index.ts`.

1. Create a dedicated Turso database with the [Turso CLI](https://docs.turso.tech/cli/installation):
   ```sh
   turso auth login
   turso db create dispute-triage
   turso db show dispute-triage --url
   turso db tokens create dispute-triage
   ```
   Provision its URL and token as `TURSO_DATABASE_URL` and `TURSO_AUTH_TOKEN` in your shell through a secret manager. Run `npm run db:init:hosted` **once before deploying**. This initializes the schema and seeds an empty database; existing disputes and audit history are preserved. It does not copy the local SQLite dataset.
2. [Import this repository into Vercel](https://vercel.com/new), choose Node 22, and retain the checked-in Vite build/output configuration. Set the five server variables in the table above: `ADMIN_USERNAME`, `ADMIN_PASSWORD_HASH`, `TURSO_DATABASE_URL`, `TURSO_AUTH_TOKEN`, and `APP_ORIGIN`. Copy the generated password hash from your private local configuration; never put the plaintext password in source or a frontend variable.
3. Set `APP_ORIGIN` to the exact production HTTPS origin (for example `https://your-project.vercel.app`, **no trailing slash**). This origin must match the hostname users visit. Other deployment aliases cannot use the API. Use a separate database and matching origin for preview deployments.
4. Deploy through Vercel's Git integration, or with an authenticated Vercel CLI using `vercel --prod`. The function refuses to start without the authentication configuration and initialized hosted database. `.vercelignore` excludes local databases, environment files, and test fixtures.
5. Verify `/api/health` returns 200, `/api/disputes` returns 401 without a session, sign-in restores access, an authenticated mutation produces an `admin` audit event, and logout prevents session reuse.

To rotate the password, generate a fresh hash in a separate directory with the configuration command, replace `ADMIN_PASSWORD_HASH` in Vercel, and redeploy. Do not reuse production secrets or production databases in preview deployments.
...
- `server/auth.ts`: password verification, persistent sessions, login throttling, and authenticated actor.
- `src/Auth.tsx`: login gate and logout; `api/index.ts`: authenticated Vercel entry point.
...
API: `GET /api/customers`, `GET /api/disputes`, `POST /api/disputes`, `GET /api/disputes/:id`, `GET /api/summary`, `PATCH /api/disputes/:id`, `POST /api/disputes/:id/notes`, `POST /api/disputes/bulk-status`. All data routes require a valid session. Writes (including login/logout) also require JSON and `X-Dispute-Client: internal-web`; patches and each bulk item require `expected_updated_at`. A note append is transactional and does not overwrite existing notes.
...
The optional remote-driver integration test uses a disposable loopback libSQL server and verifies persistence, sessions, audit actors, and logout across separate connections:

```sh
docker run -d --name triage-libsql-test -p 127.0.0.1:8089:8080 \
  ghcr.io/tursodatabase/libsql-server:v0.24.32
LIBSQL_TEST_URL=http://127.0.0.1:8089 npm test -- server/hosted.integration.test.ts
docker stop triage-libsql-test
```

It only accepts loopback servers and creates synthetic test records; it must never target a production database.
...
| Authentication | Required password hash, constant-time verification, shared login throttling, random opaque cookies, server-side revocation/expiry, password-rotation invalidation, session-derived audit actor. |
...
1. Replace the shared admin with individual identity-provider accounts, MFA, and stable actor IDs/permissions. `createApp(db, identityMiddleware)` supports injecting this; all read/write routes enforce permissions. Its default local actor is a service-test seam, not a deployment entry point; both runtime entry points explicitly supply session authentication. Add tenant/record-level access checks and authorization tests for the real identity model.
