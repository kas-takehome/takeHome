...
- Click **Add dispute** to the right of **Refresh**. Type a customer name or integer ID and choose an existing synthetic customer from the suggestions below (click, or use arrow keys and Enter). Editing the text clears the selection; unmatched text cannot create a dispute. Then enter the amount, currency and reason. Optionally select an existing agent and add a note. Successful creation refreshes the queue and totals, clears filters, and opens the new case; errors retain the form for correction. Customer loading failures offer a retry and prevent submission.
